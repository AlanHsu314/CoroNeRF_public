...
def getAtomicWeight(element):
    ...
    wgts = np.array([1.008,4.0026,6.94,9.012,10.81,12.011,14.007,
        ...
        40.078,44.955,47.867,50.94,51.996,54.938,55.845,58.6934])
...
def elvlcRead(ion_name):
    ...
    # for windows
    filename = chianti_dbase_root + os.path.sep + element + os.path.sep + ion_name + os.path.sep + ion_name + '.elvlc'
...
def wgfaRead(ion_name):
    ...
    chianti_dbase_root,chianti_version = getChiantiDir()
    element, ion = ion_name.split('_')
    filename = chianti_dbase_root + os.path.sep + element + os.path.sep + ion_name + os.path.sep + ion_name + '.wgfa'
    data,reference = readFile(filename)
...
def scupsRead(ion_name):
    ...
    chianti_dbase_root,chianti_version = getChiantiDir()
    element, ion = ion_name.split('_')
    filename = chianti_dbase_root + os.path.sep + element + os.path.sep + ion_name + os.path.sep + ion_name + '.scups'
    data,reference = readFile(filename)
...
def splupsRead(ion_name):
    ...
    chianti_dbase_root,chianti_version = getChiantiDir()
    element, ion = ion_name.split('_')
    filename = chianti_dbase_root + os.path.sep + element + os.path.sep + ion_name + os.path.sep + ion_name + '.psplups'
    data,reference = readFile(filename)
...
def abundRead(filename):
    ...
    chianti_dbase_root,chianti_version = getChiantiDir()
    if filename == None: 
        filename = chianti_dbase_root + os.path.sep + 'abundance' + os.path.sep + 'sun_photospheric_2021_asplund.abund'
...
def ioneqRead(filename):
    ...
    """
    chianti_dbase_root,chianti_version = getChiantiDir()
    filename = chianti_dbase_root + os.path.sep + 'ioneq' + os.path.sep + 'chianti.ioneq'
    ...
    widFormat = [0,3,3]
    for ss in range(nt): widFormat.append(10)
    widC = np.cumsum(widFormat)
